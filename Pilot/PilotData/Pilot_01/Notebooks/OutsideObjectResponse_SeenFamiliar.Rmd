...
```{r, echo = FALSE}
outside.hit.rate.novelty.group[, .(Mean = mean(SFAcc, na.rm = TRUE), SD = sd(SFAcc, na.rm = TRUE), N = length(SFAcc)), by = c("Group")]
t.test(SFAcc ~ Group, data = outside.hit.rate.novelty.group, paired = TRUE)
```
...
outside.hit.rate.item.order.novelty[, .(Mean = mean(SFAcc, na.rm = TRUE), SD = sd(SFAcc, na.rm = TRUE), N = length(SFAcc)), by = c("Group", "ObjOrdGrp")]

ezANOVA(outside.hit.rate.item.order.novelty, dv = .(SFAcc), wid = .(SubjectNo), within = .(Group, ObjOrdGrp))
```
...
ggplot(outside.hit.rate.per.room, aes(x = CurRating, y = SFAcc)) +
...
```{r, echo = FALSE}
baseline.rating <- lme(SFAcc ~ 1, random = ~ 1|SubjectNo, data = outside.hit.rate.per.room, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
