...
```{r, echo=FALSE, fig.align='center', fig.width=7.2, fig.height=9}
...
```{r, echo=FALSE, fig.align='center', fig.width=3.6, fig.height=3, fig.cap='Figure1. Mean accuracy for item memory respectively for Familiar and Novel rooms.'}
ggplot(OutsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict, .(MeanItemAccuracy = mean(ItemAccuracy)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanItemAccuracy)) +
...
```{r, echo = FALSE}
baseline.item <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo/Group, data = Curiosity.Recall.Old, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(SeenHit ~ CuriosityRatingType * ItemOrderType, random = ~ 1| SubjectNo/CuriosityRatingType/ItemOrderType, data = Curiosity.Recall.Old.Collapsed, method = "ML", control = list(msMaxIter = 500, opt = "optim"))
...
ggplot(Curiosity.Recall.Old.Collapsed.Alt.Cur, aes(x = RatingGroup, y = SeenHit)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
...
``` {r, echo = FALSE}
baseline.rating <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo/Group, data = OutsideObjectsMemory, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
