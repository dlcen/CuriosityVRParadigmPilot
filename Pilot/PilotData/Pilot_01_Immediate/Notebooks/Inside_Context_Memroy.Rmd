...
> This notebook focuses on context memory inside the rooms for the intermediate version. 
...
```
The difference didn't reach significance, but we only had 12 participants. 
...
```
For *familiar* room, the objects presented earlier were remembered better. For the *novel* rooms there is not much change with the item order. `r text_spec("Note that we only have 8 participants here", background = "#D05A6E", color = "white", bold = T, font_size = 15)`. 
...
baseline.context <- lme(ContextAccuracy ~ 1, random = ~ 1|SubjectNo, data = Encoding.Recall.Old, method = "ML", control = list(msMaxIter = 1000, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
Including $\small Novelty$ marginally significantly improved the model fit ($p = 0.079$), suggesting it may play an important role in describing the data.
...
In the complete model that contains all the predictors, none of the predictors has a significant estimated parameter value. It may be mainly we had only 12 participants now and less power due to small prediction-participant ratio. Below I will examine the optimal model `novelty.model.1`. It's considered as "optimal" because adding more predictors didn't improve its model fit.
...
There seems to be some difference between the *novel* and *familiar* rooms at the intercept (the lowest curiosity rating). `r text_spec("Will wait for more participants to see how the pattern changes.", background = "#D05A6E", color = "white", bold = T, font_size = 15)`
...
```
Against our prediction, the accuracy of context memory *decreases* as the curiosity rating increases.
...
```
No significant improvement to the model fit after including *curiosity rating*.
...
The estimated value for the parameter $CuriosityRating$ is negative, as what we observed on Figure 3, suggesting a negative coorelation. However, the correlation was not significant. `r text_spec("Will wait for more participants to see how this relationship changes", background = "#D05A6E", color = "white", bold = T, font_size = 15)`.

## Does the relationship with curiosity rating depend on item order?
Maybe the correlation between curiosity rating and context memory depends on the order at which the item was seen.
...
ggplot(Encoding.Recall.Old, aes(x = CuriosityRating, y = ContextAccuracy)) +
  ...
  theme(strip.text = element_text(face = "bold", size = 11),
        axis.title = element_text(face = "bold"))
```
A trend from positive to negative.
