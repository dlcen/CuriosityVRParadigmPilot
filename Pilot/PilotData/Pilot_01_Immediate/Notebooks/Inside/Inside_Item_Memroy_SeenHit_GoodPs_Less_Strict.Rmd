...
> This notebook focuses on item memory inside the rooms. 
...
```{r, echo = FALSE, message=FALSE, warning=FALSE}
# load("~/Documents/OneDrive - Cardiff University/ProjectsVRCuriosityMemory/PilotData/RawData/Pilot_01/.RData")
...
``` {r}
Encoding.Recall.Freq.Grp$Type <- "Good"
Encoding.Recall.Freq.Grp[SubjectNo %in% bad.ps.less.strict]$Type <- "Bad"

Encoding.Recall.Freq.Rsp$Type <- "Good"
Encoding.Recall.Freq.Rsp[SubjectNo %in% bad.ps.less.strict]$Type <- "Bad"
```

```{r, echo=FALSE, fig.align='center', fig.width=6.4, fig.height=7.2, fig.cap='Figure 1. Response frequency for new and old items respectively for each individual participant.'}
ggplot(Encoding.Recall.Freq.Rsp, aes(x = Response, y = Frequency, group = CorrObjResp)) + theme_bw() +
  geom_rect(data = subset(Encoding.Recall.Freq.Rsp,Type == 'Bad'),aes(fill = Type),xmin = -Inf,xmax = Inf,ymin = -Inf,ymax = Inf,alpha = 0.1) +
...
```{r, echo=FALSE, fig.align='center', fig.width=7.2, fig.height=7.2, fig.cap='Figure 2. Response frequency for new and old items respectively for each Familiar/Novel group and each individual participant.'}
ggplot(Encoding.Recall.Freq.Grp, aes(x = Response, y = Frequency, group = Group)) + theme_bw() +
  geom_rect(data = subset(Encoding.Recall.Freq.Grp,Type == 'Bad'),aes(fill = Type),xmin = -Inf,xmax = Inf,ymin = -Inf,ymax = Inf,alpha = 0.1) +
...
ggplot(InsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict, .(MeanSeenHit = mean(SeenHit)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanSeenHit)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  labs(x = "Room novelty", y = "Mean accuracy for item memory")
```


```{r, echo = FALSE}
t.test(MeanSeenHit ~ Group, data = InsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict, .(MeanSeenHit = mean(SeenHit)), by = c("SubjectNo", "Group")], paired = TRUE)
```

## Item memory as a function of item order
### Visualise the data
```{r, echo = FALSE, fig.align= 'center', fig.width=4.8, fig.height=3.6, fig.cap="Figure 4. Mean hit rate for seen response for items at each order (i.e., 1st, 2nd, 3rd, etc), respectively for familiar and novel rooms."}
ggplot(Encoding.Recall.Old[!SubjectNo %in% bad.ps.less.strict], aes(x = ItemOrder, y = SeenHit)) +
  ...
  labs(x = "Item order", y = "Hit rate of recollection") +
...
Build models with item memory accuracy as $y$ and iexamine effects of item order and novelty by comparing the models. 
...
```{r, echo = FALSE}
baseline.item <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo/Group, data = Encoding.Recall.Old[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
anova(order.model.linear.1, order.model.linear.2, order.model.quad.1, novelty.model.1, novelty.model.2, novelty.model.3)
```
...
In the complete model that contains all the predictors, none of the predictors has a significant estimated parameter value. It may be mainly we had 21 participants now and less power due to small prediction-participant ratio.
...
ggplot(Encoding.Recall.Old.Collapsed.Grp[!SubjectNo %in% bad.ps.less.strict], aes(x = Group, y = SeenHit)) +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(SeenHit ~ Group * ItemOrderType, random = ~ 1| SubjectNo/Group/ItemOrderType, data = Encoding.Recall.Old.Collapsed.Grp[!SubjectNo %in% bad.ps.less.strict], method = "ML")
...
ggplot(InsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict], aes(x = CuriosityRating, y = SeenHit)) +
...
```{r, echo = FALSE}
baseline.rating <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo, data = InsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
```
Including $\small CuriosityRating$ did not improve the model fit significantly, so may not play an important role. 
...
```
After taking into account individual difference, the estimated slope (relationship between curiosity rating and item memory) is weakly negative 

## Does the relationship with curiosity rating depend on item order?
```{r, echo=FALSE, fig.align='center', fig.width=10.8, fig.height=3, fig.cap='Figure 6. Mean accuracy across participants for item memory as a function of curiosity rating for each order at which the item was seen (I.e., 1st, 2nd, 3rd, etc.).'}
ggplot(Encoding.Recall.Old[!SubjectNo %in% bad.ps.less.strict], aes(x = CuriosityRating, y = SeenHit)) +
  ...
  theme(strip.text = element_text(face = "bold", size = 11),
        axis.title = element_text(face = "bold"))
```
For four of the six orders, there seems to be a positive correlates between curiosity rating and item memory.
...
```{r}
baseline.item <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo, data = Encoding.Recall.Old[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
```
Including none of the predictors improved the model fit significantly except the last one.
...
Interestingly, there is a significant interaction between Curiosity and item order.

#### Was the relationship significantly positive for the first object?
```{r}
baseline.item.first <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo, data = Encoding.Recall.Old[!SubjectNo %in% bad.ps.less.strict & ItemOrder == 1], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)

rating.intercept.first <- update(baseline.item.first, .~. + CuriosityRating)
rating.slope.first     <- update(rating.intercept.first, random = ~ 1 + CuriosityRating|SubjectNo)

anova(baseline.item.first, rating.intercept.first, rating.slope.first)
```

```{r}
summary(rating.intercept.first)
```


### Collapse the order and curiosity rating
```{r, echo=FALSE, fig.align='center', fig.width=4.2, fig.height=3.6, fig.cap='Figure 7. Mean hit rate as a function of item order (Early vs. Late) and curiosity (Low vs. High).'}
ggplot(Encoding.Recall.Old.Collapsed[!SubjectNo %in% bad.ps.less.strict], aes(x = CuriosityRatingType, y = SeenHit)) +
  stat_summary(fun.y = mean, geom = "bar", size = 2, fill = "grey50") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.9)) +
  labs(x = "Curiosity rating", y = "Hit rate for recollection") +
  facet_wrap( ~ ItemOrderType, nrow = 1) +
...
ggplot(Encoding.Recall.Old.Collapsed.Alt.Cur[!SubjectNo %in% bad.ps.less.strict], aes(x = RatingGroup, y = SeenHit)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
...
```{r, echo = FALSE}
t.test(SeenHit ~ RatingGroup, data = Encoding.Recall.Old.Collapsed.Alt.Cur[!SubjectNo %in% bad.ps.less.strict], paired = T)
```

```{r, echo=FALSE, fig.align='center', fig.width=4.2, fig.height=3.6}
ggplot(Encoding.Recall.Old.Collapsed.Alt[!SubjectNo %in% bad.ps.less.strict], aes(x = RatingGroup, y = SeenHit)) +
  stat_summary(fun.y = mean, geom = "bar", size = 2, fill = "grey50") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.9)) +
  labs(x = "Curiosity rating", y = "Hit rate for recollection") +
  facet_wrap( ~ ItemOrderType, nrow = 1) +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(SeenHit ~ RatingGroup * ItemOrderType, random = ~ 1| SubjectNo/RatingGroup/ItemOrderType, data = Encoding.Recall.Old.Collapsed.Alt[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
...
``` {r, echo = FALSE}
baseline.rating <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo/Group, data = InsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
```{r, echo=FALSE}
novelty.model <- update(baseline.rating, .~. + Group)
...
anova(baseline.rating, novelty.model, rating.model.1, rating.model.2, interaction.model)
```

### Check the model
```{r, echo = FALSE}
summary(rating.model.2)
```


## Check the collapsed data
```{r, echo=FALSE, fig.align='center', fig.width=4.2, fig.height=3.6}
ggplot(Encoding.Recall.Old.Collapsed.Interaction[!SubjectNo %in% bad.ps.less.strict], aes(x = CuriosityRatingType, y = SeenHit)) +
  stat_summary(fun.y = mean, geom = "bar", size = 2, fill = "grey50") +
...
#### Stats on it
```{r, echo = FALSE}
interactionModel <- lme(SeenHit ~ CuriosityRatingType * Group, random = ~ 1| SubjectNo/CuriosityRatingType/Group, data = Encoding.Recall.Old.Collapsed.Interaction[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
...
```{r, echo=FALSE, fig.align='center', fig.width=4.2, fig.height=3.6}
ggplot(Encoding.Recall.Old.Collapsed.Alt.Interaction[!SubjectNo %in% bad.ps.less.strict], aes(x = RatingGroup, y = SeenHit)) +
  stat_summary(fun.y = mean, geom = "bar", size = 2, fill = "grey50") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.9)) +
  labs(x = "Curiosity rating", y = "Hit rate for recollection") +
  facet_wrap( ~ Group, nrow = 1) +
...
```{r, echo = FALSE}
interactionModel <- lme(SeenHit ~ RatingGroup * Group, random = ~ 1| SubjectNo/RatingGroup/Group, data = Encoding.Recall.Old.Collapsed.Alt.Interaction[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
...
ggplot(InsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict & Group == "Novel"], aes(x = CuriosityRating, y = SeenHit)) +
  stat_summary(fun.data = mean_cl_boot, na.rm = T, geom = "errorbar", width = 0.2) +
  stat_summary(fun.y = mean, na.rm = T, geom = "point", size = 2) +
  stat_smooth(method = "lm", se = FALSE) +
  scale_x_continuous(breaks = c(1:7)) +
