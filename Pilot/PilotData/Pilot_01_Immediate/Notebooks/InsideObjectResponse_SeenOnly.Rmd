...
MeanSAccNoveltyCmpPlot <- ggplot(inside.hit.rate.novelty.group, aes(x = Group, y = SAcc)) +
...
MeanSAccCurGrpOrdPlot <- ggplot(inside.hit.rate.item.order.curiosity[!SubjectNo %in% bad.ps.less.strict & !is.na(CurGrp)], aes(x = CurGrp, y = SAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  facet_wrap( ~ ObjOrdGrp) + 
  labs(x = "Curiosity group", y = "Corrected hit rate for \"seen\"")
...
baseline  <- lme(SAcc ~ 1, random = ~ 1 + CurGrp + ObjOrdGrp|SubjectNo, data = inside.hit.rate.item.order.curiosity[!is.na(CurGrp)], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
```{r, echo=FALSE}
base.model    <- lme(SAcc ~ 1, random = ~ 1 + CurGrp + Group | SubjectNo, data = inside.hit.rate.per.room[!is.na(CurGrp)], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
...
### Curiosity first, novelty next
```{r, echo=FALSE}
base.model    <- lme(SAcc ~ 1, random = ~ 1 + CurGrpMedian + Group | SubjectNo, data = inside.hit.rate.per.room[!is.na(CurGrpMedian)], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
...
### Curiosity first, novelty next
```{r}
basic <- lme(SAcc ~ 1, random = ~ 1 + Group|SubjectNo, data = inside.hit.rate.novelty.group, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
```{r}
fit.1 <- lme(SAcc ~ Group, random = ~ 1 + Group|SubjectNo, data = inside.hit.rate.novelty.group, method = "ML")
summary(fit.1)
```

```{r}
fit.2 <- lme(MeanCurRt ~ Group, random = ~ 1 + Group|SubjectNo, data = inside.hit.rate.novelty.group, method = "ML")
summary(fit.2)
```

```{r}
fit.3 <- lme(SAcc ~ MeanCurRt + Group, random = ~ 1 + Group|SubjectNo, data = inside.hit.rate.novelty.group, method = "ML")
summary(fit.3)
```
