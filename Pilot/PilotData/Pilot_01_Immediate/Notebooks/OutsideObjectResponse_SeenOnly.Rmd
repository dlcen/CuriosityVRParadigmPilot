...
```{r echo=FALSE}
outside.hit.rate.curiosity.group.median$CurGrp <- factor(outside.hit.rate.curiosity.group.median$CurGrp)
outside.hit.rate.curiosity.group.median$CurGrp <- factor(outside.hit.rate.curiosity.group.median$CurGrp, levels = levels(outside.hit.rate.curiosity.group.median$CurGrp)[c(2, 1)])
```

```{r, echo=FALSE, warning=FALSE, fig.align='center', fig.width=3.6, fig.height=3}
MeanSAccCurGrpCmpPlot <- ggplot(outside.hit.rate.curiosity.group.median[!SubjectNo %in% bad.ps.less.strict], aes(x = CurGrp, y = SAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  labs(x = "Curiosity group", y = "Corrected hit rate for \"seen\"")
...
```{r, echo=FALSE}
cur.grp.model <- lme(SAcc ~ CurGrp,random = ~ 1 + CurGrp|SubjectNo,  data = outside.hit.rate.curiosity.group.median, method = "ML")
...
```{r echo=FALSE}
outside.hit.rate.item.order.curiosity.median$CurGrp <- factor(outside.hit.rate.item.order.curiosity.median$CurGrp)
outside.hit.rate.item.order.curiosity.median$CurGrp <- factor(outside.hit.rate.item.order.curiosity.median$CurGrp, levels = levels(outside.hit.rate.item.order.curiosity.median$CurGrp)[c(2, 1)])
```

```{r, echo=FALSE, warning=FALSE, fig.align='center', fig.width=6.4, fig.height=3}
MeanSAccCurGrpOrdPlot <- ggplot(outside.hit.rate.item.order.curiosity.median[!SubjectNo %in% bad.ps.less.strict & !is.na(CurGrp)], aes(x = CurGrp, y = SAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  facet_wrap( ~ ObjOrdGrp) + 
  labs(x = "Curiosity group", y = "Corrected hit rate for \"seen\"")
...
```{r, echo = FALSE, warning=FALSE}
outside.hit.rate.item.order.curiosity.median[, .(Mean = mean(SAcc, na.rm = TRUE), SD = sd(SAcc, na.rm = TRUE), N = length(SAcc)), by = c("CurGrp", "ObjOrdGrp")]

baseline  <- lme(SAcc ~ 1, random = ~ 1 + CurGrp + ObjOrdGrp|SubjectNo, data = outside.hit.rate.item.order.curiosity.median[!is.na(CurGrp)], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
ord.model <- update(baseline,  .~. + ObjOrdGrp)
cur.model <- update(ord.model, .~. + CurGrp)
int.model <- update(cur.model, .~. + ObjOrdGrp : CurGrp)
