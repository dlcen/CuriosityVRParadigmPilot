...
MeanSrcHitCurGrpOrdPlot <- ggplot(outside.hit.rate.item.order.curiosity[!is.na(CurGrp)], aes(x = CurGrp, y = SrcHit)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  facet_wrap( ~ ObjOrdGrp) + 
  labs(x = "Curiosity group", y = "Hit rate for source memory")
...
```{r, echo = FALSE}
outside.hit.rate.item.order.curiosity[!is.na(CurGrp), .(Mean = mean(SrcHit, na.rm = TRUE), SD = sd(SrcHit, na.rm = TRUE), N = length(SrcHit)), by = c("CurGrp", "ObjOrdGrp")]
```

```{r, echo=FALSE}
baseline  <- lme(SrcHit ~ 1, random = ~ 1 + CurGrp + ObjOrdGrp|SubjectNo, data = outside.hit.rate.item.order.curiosity[!is.na(CurGrp)], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
contrast.matrix <- rbind(
        "Early: High - Low"  = c(0, 0, 1, 0),
        "Late:  High - Low"  = c(0, 0, 1, 1)
)
...
```{r, echo=FALSE, warning=FALSE, fig.align='center', fig.width=3.6, fig.height=3}
MeanSrcHitCurGrpCmpPlot <- ggplot(outside.hit.rate.curiosity.group.median, aes(x = CurGrp, y = SrcHit)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  labs(x = "Curiosity group", y = "Hit rate for source memory")
...
```{r, echo=FALSE}
cur.grp.model <- lme(SrcHit ~ CurGrp,random = ~ 1 + CurGrp|SubjectNo,  data = outside.hit.rate.curiosity.group.median, method = "ML")
summary(cur.grp.model)
```
...
```{r, echo=FALSE, warning=FALSE, fig.align='center', fig.width=6.4, fig.height=3}
MeanSrcHitCurGrpOrdPlot <- ggplot(outside.hit.rate.item.order.curiosity.median[!is.na(CurGrpMedian)], aes(x = CurGrpMedian, y = SrcHit)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  facet_wrap( ~ ObjOrdGrp) + 
  labs(x = "Curiosity group", y = "Hit rate for source memory")
...
```{r, echo = FALSE, warning=FALSE}
outside.hit.rate.item.order.curiosity.median[!is.na(CurGrpMedian), .(Mean = mean(SrcHit, na.rm = TRUE), SD = sd(SrcHit, na.rm = TRUE), N = length(SrcHit)), by = c("CurGrpMedian", "ObjOrdGrp")]
