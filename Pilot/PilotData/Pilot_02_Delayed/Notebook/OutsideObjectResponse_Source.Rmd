...
There are `r n.negative.SrcAcc` negative value out of a total of `r nrow(outside.hit.rate.novelty.group)` participants. 
...
There are `r n.negative.SrcAcc` negative values out of a total of `r nrow(outside.hit.rate.item.order.novelty)` participants.
...
##### Early
```{r, echo=FALSE}
t.test(SrcAcc ~ Group, data = outside.hit.rate.item.order.novelty[ObjOrdGrp == "Early"], paired = T)
```

##### Later
```{r, echo=FALSE}
t.test(SrcAcc ~ Group, data = outside.hit.rate.item.order.novelty[ObjOrdGrp == "Later"], paired = T)
```
...
MeanSrcAccCurGrpCmpPlot <- ggplot(outside.hit.rate.curiosity.group, aes(x = CurGrp, y = SrcAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
...
```{r, echo = FALSE}
outside.hit.rate.curiosity.group[, .(Mean = mean(SrcAcc, na.rm = TRUE), SD = sd(SrcAcc, na.rm = TRUE), N = length(SrcAcc)), by = c("CurGrp")]
...
* **Is there any corrected hit rate < 0?**
```{r, echo=FALSE}
n.negative.SrcAcc <- nrow(outside.hit.rate.item.order.curiosity[SrcAcc < 0])
```

  * There are `r n.negative.SrcAcc` negative values out of a total of `r nrow(outside.hit.rate.item.order.curiosity)` values.

```{r, echo=FALSE}
outside.hit.rate.item.order.curiosity[SrcAcc < 0, ]
```
...
MeanSrcAccCurGrpOrdPlot <- ggplot(outside.hit.rate.item.order.curiosity, aes(x = CurGrp, y = SrcAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  facet_wrap( ~ ObjOrdGrp) + 
  labs(x = "Curiosity group", y = "corrected hit rate")
...
```{r, echo = FALSE}
outside.hit.rate.item.order.curiosity[, .(Mean = mean(SrcAcc, na.rm = TRUE), SD = sd(SrcAcc, na.rm = TRUE), N = length(SrcAcc)), by = c("CurGrp", "ObjOrdGrp")]
```
...
```{r, echo=FALSE, warning=FALSE, fig.align='center', fig.width=3.6, fig.height=3}
MeanSrcAccCurGrpCmpPlot <- ggplot(outside.hit.rate.curiosity.group.median, aes(x = CurGrpMedian, y = SrcAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  labs(x = "Curiosity group", y = "Corrected hit rate")
...
```{r, echo=FALSE}
cur.grp.model <- lme(SrcAcc ~ CurGrpMedian,random = ~ 1 + CurGrpMedian|SubjectNo,  data = outside.hit.rate.curiosity.group.median, method = "ML")
summary(cur.grp.model)
```
...
* **Is there any corrected hit rate < 0?**
```{r, echo=FALSE}
n.negative.SrcAcc <- nrow(outside.hit.rate.item.order.curiosity.median[SrcAcc < 0])
```
...
```{r, echo=FALSE}
outside.hit.rate.item.order.curiosity.median[SrcAcc < 0 , ]
```

```{r, echo=FALSE}
# , eval=FALSE, include=FALSE}
outside.hit.rate.item.order.curiosity.median[SrcAcc < 0]$SrcAcc <- 0
```

```{r, echo=FALSE, warning=FALSE, fig.align='center', fig.width=6.4, fig.height=3}
MeanSrcAccCurGrpOrdPlot <- ggplot(outside.hit.rate.item.order.curiosity.median, aes(x = CurGrpMedian, y = SrcAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  facet_wrap( ~ ObjOrdGrp) + 
  labs(x = "Curiosity group", y = "Corrected hit rate")
...
```{r, echo = FALSE, warning=FALSE}
outside.hit.rate.item.order.curiosity.median[, .(Mean = mean(SrcAcc, na.rm = TRUE), SD = sd(SrcAcc, na.rm = TRUE), N = length(SrcAcc)), by = c("CurGrpMedian", "ObjOrdGrp")]
