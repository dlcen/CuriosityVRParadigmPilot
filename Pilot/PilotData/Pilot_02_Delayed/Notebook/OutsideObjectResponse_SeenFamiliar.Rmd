...
outside.hit.rate.novelty.group[SFAcc < 0]$SFAcc <- 0
...
MeanSFAccNoveltyCmpPlot <- ggplot(outside.hit.rate.novelty.group[!SubjectNo %in% bad.ps.less.strict], aes(x = Group, y = SFAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  labs(x = "Room novelty", y = "corrected hit rate for \"seen\" and \"familiar\"")

MeanSFAccNoveltyCmpPlot
```

```{r, echo = FALSE}
outside.hit.rate.novelty.group[!SubjectNo %in% bad.ps.less.strict, .(Mean = mean(SFAcc, na.rm = TRUE), SD = sd(SFAcc, na.rm = TRUE), N = length(SFAcc)), by = c("Group")]
t.test(SFAcc ~ Group, data = outside.hit.rate.novelty.group[!SubjectNo %in% bad.ps.less.strict], paired = TRUE)
```

## Any difference between *Early* and *Later* object groups?
```{r, echo = FALSE}
load("../GroupData/OutsideObjectResponseItemOrders.RData")
outside.hit.rate.item.order.novelty[SFAcc < 0]$SFAcc <- 0
...
MeanSFAccNoveltyTmpOrdCmpPlot <- ggplot(outside.hit.rate.item.order.novelty[!SubjectNo %in% bad.ps.less.strict], aes(x = Group, y = SFAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
...
outside.hit.rate.item.order.novelty[!SubjectNo %in% bad.ps.less.strict, .(Mean = mean(SFAcc, na.rm = TRUE), SD = sd(SFAcc, na.rm = TRUE), N = length(SFAcc)), by = c("Group", "ObjOrdGrp")]

ezANOVA(outside.hit.rate.item.order.novelty[!SubjectNo %in% bad.ps.less.strict], dv = .(SFAcc), wid = .(SubjectNo), within = .(Group, ObjOrdGrp))
```
...
outside.hit.rate.per.room[SFAcc < 0]$SFAcc <- 0
outside.hit.rate.curiosity.group[SFAcc < 0]$SFAcc <- 0
outside.hit.rate.item.order.curiosity[SFAcc < 0]$SFAcc <- 0
```
  
## Mean accuracy of item memory as a function of curiosity rating
```{r, echo=FALSE, fig.align='center', fig.width=3.6, fig.height=3}
ggplot(outside.hit.rate.per.room[!SubjectNo %in% bad.ps.less.strict], aes(x = CurRating, y = SFAcc)) +
  ...
  labs(x = "Curiosity rating", y = "corrected hit rate for \"seen\" and \"familiar\"") 
```
...
```{r, echo = FALSE}
baseline.rating <- lme(SFAcc ~ 1, random = ~ 1|SubjectNo, data = outside.hit.rate.per.room[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
MeanSFAccCurGrpCmpPlot <- ggplot(outside.hit.rate.curiosity.group[!SubjectNo %in% bad.ps.less.strict], aes(x = CurGrp, y = SFAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
...
MeanSFAccCurGrpOrdPlot <- ggplot(outside.hit.rate.item.order.curiosity[!SubjectNo %in% bad.ps.less.strict & !is.na(CurGrp)], aes(x = CurGrp, y = SFAcc)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  facet_wrap( ~ ObjOrdGrp) + 
  labs(x = "Curiosity group", y = "Corrected hit rate for \"seen\" and \"familiar\"")
...
```{r, echo = FALSE, warning=FALSE}
outside.hit.rate.item.order.curiosity[!SubjectNo %in% bad.ps.less.strict, .(Mean = mean(SFAcc, na.rm = TRUE), SD = sd(SFAcc, na.rm = TRUE), N = length(SFAcc)), by = c("CurGrp", "ObjOrdGrp")]

baseline  <- lme(SFAcc ~ 1, random = ~ 1 + CurGrp + ObjOrdGrp|SubjectNo, data = outside.hit.rate.item.order.curiosity[!is.na(CurGrp) & !SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
