...
t.test(SAcc ~ Group, data = outside.hit.rate.novelty.group[!SubjectNo %in% bad.ps.less.strict], paired = TRUE)
```
...
outside.hit.rate.item.order.novelty[!SubjectNo %in% bad.ps.less.strict, .(Mean = mean(SAcc, na.rm = TRUE), SD = sd(SAcc, na.rm = TRUE), N = length(SAcc)), by = c("Group", "ObjOrdGrp")]

ezANOVA(outside.hit.rate.item.order.novelty[!SubjectNo %in% bad.ps.less.strict], dv = .(SAcc), wid = .(SubjectNo), within = .(Group, ObjOrdGrp))
```
...
ggplot(outside.hit.rate.per.room[!SubjectNo %in% bad.ps.less.strict], aes(x = CurRating, y = SAcc)) +
...
```{r, echo = FALSE}
baseline.rating <- lme(SAcc ~ 1, random = ~ 1|SubjectNo, data = outside.hit.rate.per.room[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
```{r, echo = FALSE}
outside.hit.rate.curiosity.group[!SubjectNo %in% bad.ps.less.strict, .(Mean = mean(SAcc, na.rm = TRUE), SD = sd(SAcc, na.rm = TRUE), N = length(SAcc)), by = c("CurGrp")]

summary(lme( SAcc ~ CurGrp, random = ~ 1 + CurGrp|SubjectNo, data = outside.hit.rate.curiosity.group[!SubjectNo %in% bad.ps.less.strict], method = "ML", na.action = na.exclude))
```
...
```{r, echo = FALSE, warning=FALSE}
outside.hit.rate.item.order.curiosity[!SubjectNo %in% bad.ps.less.strict, .(Mean = mean(SAcc, na.rm = TRUE), SD = sd(SAcc, na.rm = TRUE), N = length(SAcc)), by = c("CurGrp", "ObjOrdGrp")]

baseline  <- lme(SAcc ~ 1, random = ~ 1 + CurGrp + ObjOrdGrp|SubjectNo, data = outside.hit.rate.item.order.curiosity[!is.na(CurGrp) & !SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
