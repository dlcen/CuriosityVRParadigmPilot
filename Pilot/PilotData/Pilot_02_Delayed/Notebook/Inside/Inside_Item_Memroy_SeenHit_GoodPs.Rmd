...
for (this.p in participant.folders) {
  this.data <- Encoding.Recall.Freq.Grp[SubjectNo == this.p]
...
Encoding.Recall.Freq.Grp[SubjectNo %in% bad.ps]$Type <- "Bad"

Encoding.Recall.Freq.Rsp$Type <- "Good"
Encoding.Recall.Freq.Rsp[SubjectNo %in% bad.ps]$Type <- "Bad"
```
...
ggplot(InsideObjectsMemory[, .(MeanSeenHit = mean(SeenHit)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanSeenHit)) +
...
```{r, echo = FALSE}
t.test(MeanSeenHit ~ Group, data = InsideObjectsMemory[, .(MeanSeenHit = mean(SeenHit)), by = c("SubjectNo", "Group")], paired = TRUE)
```
...
ggplot(Encoding.Recall.Old, aes(x = ItemOrder, y = SeenHit)) +
...
```
For both *familiar* and *novel* rooms, the hit rate seems to be higher at the two ends as compared to the middle. `r text_spec("Note that we now have 21 participants here", background = "#D05A6E", color = "white", bold = T, font_size = 15)`. 
...
```{r, echo = FALSE}
baseline.item <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo, data = Encoding.Recall.Old, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(Encoding.Recall.Old.Collapsed.Grp, aes(x = Group, y = SeenHit)) +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(SeenHit ~ Group * ItemOrderType, random = ~ Group + ItemOrderType | SubjectNo, data = Encoding.Recall.Old.Collapsed.Grp, method = "ML")
...
ggplot(InsideObjectsMemory, aes(x = CuriosityRating, y = SeenHit)) +
...
```{r, echo = FALSE}
baseline.rating <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo, data = InsideObjectsMemory, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(Encoding.Recall.Old, aes(x = CuriosityRating, y = SeenHit)) +
...
### Collapse the order and curiosity rating
```{r, echo=FALSE, fig.align='center', fig.width=4.2, fig.height=3.6, fig.cap='Figure 7. Mean hit rate as a function of item order (Early vs. Late) and curiosity (Low vs. High).'}
ggplot(Encoding.Recall.Old.Collapsed, aes(x = CuriosityRatingType, y = SeenHit)) +
  stat_summary(fun.y = mean, geom = "bar", size = 2, fill = "grey50") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.9)) +
  labs(x = "Curiosity rating", y = "Hit rate for recollection") +
  facet_wrap( ~ ItemOrderType, nrow = 1) +
