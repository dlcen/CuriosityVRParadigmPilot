...
```{r, echo=FALSE, fig.align='center', fig.width=4.8, fig.height=4.2, fig.cap='Figure2. Frequency of correctly labelled distractor objects for each individual participant.'}
...
```{r meanbars, echo=FALSE, fig.align='center', fig.width=3.6, fig.height=3, fig.cap='Figure4. Mean accuracy for context memory respectively for Familiar and Novel rooms.'}
ggplot(InsideObjectsMemory[, .(MeanContextAccuracy = mean(ContextAccuracy)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanContextAccuracy)) +
...
```{r, echo = FALSE}
t.test(MeanContextAccuracy ~ Group, data = InsideObjectsMemory[, .(MeanContextAccuracy = mean(ContextAccuracy)), by = c("SubjectNo", "Group")], paired = TRUE)
```
The difference didn't reach significance. Now we have 29 participants. Maybe more participants? 
...
ggplot(Encoding.Recall.Old, aes(x = ItemOrder, y = ContextAccuracy)) +
...
```
For *novel* rooms, the first and last objects were remembered better and the memory for the middle ones were the worst. For the *familiar* rooms, the performance didn't change much if not decreasing. `r text_spec("Note that we have 29 participants here", background = "#D05A6E", color = "white", bold = T, font_size = 15)`. 
...
```{r, echo = FALSE}
baseline.context <- lme(ContextAccuracy ~ 1, random = ~ 1|SubjectNo, data = Encoding.Recall.Old, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(Encoding.Recall.Old.Collapsed.Grp, aes(x = Group, y = ContextAccuracy)) +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(ContextAccuracy ~ Group * ItemOrderType, random = ~ Group + ItemOrderType | SubjectNo, data = Encoding.Recall.Old.Collapsed.Grp, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
...
ggplot(InsideObjectsMemory, aes(x = CuriosityRating, y = ContextAccuracy)) +
...
```{r, echo = FALSE}
baseline.rating <- lme(ContextAccuracy ~ 1, random = ~ 1|SubjectNo, data = InsideObjectsMemory, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(ContextAccuracy ~ CuriosityRatingType * ItemOrderType, random = ~ CuriosityRatingType + ItemOrderType | SubjectNo, data = Encoding.Recall.Old.Collapsed, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
