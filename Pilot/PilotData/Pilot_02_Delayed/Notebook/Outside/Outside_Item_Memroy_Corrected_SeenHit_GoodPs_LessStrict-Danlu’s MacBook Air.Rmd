...
curiosityModel     <- update(orderModel.int,   .~. + CuriosityRatingType)
interactionModel   <- update(curiosityModel,   .~. + CuriosityRatingType : ItemOrder)

anova(baseline, orderModel.int, curiosityModel, interactionModel)
```

There was a main effect of item order, ${\chi}^2$(1) = 6.55, *p* = 0.011, and a main effect of curiosity rating (high vs. low), ${\chi}^2$(1) = 4.15, *p* = 0.042.
...
```{r}
baseline           <- lme(CorrSeenHit ~ 1, random = ~ 1|SubjectNo/RatingGroup, data = Curiosity.Recall.Old.CuriosityType.MeanSep[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim"))

orderModel.int     <- update(baseline,         .~. + ItemOrder)
# orderModel.slp     <- update(orderModel.int,   random = ~ 1 + ItemOrder|SubjectNo/RatingGroup )

curiosityModel     <- update(orderModel.int,   .~. + RatingGroup)
interactionModel   <- update(curiosityModel,   .~. + RatingGroup : ItemOrder)

anova(baseline, orderModel.int, curiosityModel, interactionModel)
```
There was a main effect of item order, ${\chi}^2$(1) = 6.16, *p* = 0.013, and a marginal main effect of curiosity rating (high vs. low), ${\chi}^2$(1) = 3.56, *p* = 0.059.
