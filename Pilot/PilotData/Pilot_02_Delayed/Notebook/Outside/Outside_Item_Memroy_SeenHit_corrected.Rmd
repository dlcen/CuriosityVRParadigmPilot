...
ggplot(OutsideObjectsMemory[, .(MeanSeenHit = mean(CorrSeenHit)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanSeenHit)) +
...
```{r, echo = FALSE}
t.test(MeanSeenHit ~ Group, data = OutsideObjectsMemory[, .(MeanSeenHit = mean(CorrSeenHit)), by = c("SubjectNo", "Group")], paired = TRUE)
```
...
```
For *familiar* rooms the accuracy of item memory increases with the item order, but the pattern was not obvious for the *novel* rooms. `r text_spec("Note that we only have 17 participants here", background = "#D05A6E", color = "white", bold = T, font_size = 15)`. 
