...
```{r, echo=FALSE, fig.align='center', fig.width=6.4, fig.height=8.1, fig.cap='Figure 1. Response frequency for new and old items respectively for each individual participant.'}
...
```{r, echo=FALSE, fig.align='center', fig.width=7.2, fig.height=8.1, fig.cap='Figure 2. Response frequency for new and old items respectively for each Familiar/Novel group and each individual participant.'}
...
```{r, echo = FALSE}
t.test(MeanItemAccuracy ~ Group, data = OutsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict, .(MeanItemAccuracy = mean(ItemAccuracy)), by = c("SubjectNo", "Group")], paired = TRUE)
```
The difference didn't reach significance, but we only had 23 good participants. 
...
ggplot(Curiosity.Recall.Old[!SubjectNo %in% bad.ps.less.strict], aes(x = ItemOrder, y = Accuracy)) +
...
```
For *familiar* rooms the accuracy of item memory increases with the item order, but the pattern was not obvious for the *novel* rooms. `r text_spec("Note that we only have 23 participants here", background = "#D05A6E", color = "white", bold = T, font_size = 15)`. 
...
```{r, echo = FALSE}
baseline.item <- lme(Accuracy ~ 1, random = ~ 1|SubjectNo, data = Curiosity.Recall.Old[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(OutsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict], aes(x = CuriosityRating, y = ItemAccuracy)) +
...
```{r, echo = FALSE}
baseline.rating <- lme(ItemAccuracy ~ 1, random = ~ 1|SubjectNo, data = OutsideObjectsMemory[!SubjectNo %in% bad.ps.less.strict], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(Curiosity.Recall.Old[!SubjectNo %in% bad.ps.less.strict], aes(x = CuriosityRating, y = Accuracy)) +
