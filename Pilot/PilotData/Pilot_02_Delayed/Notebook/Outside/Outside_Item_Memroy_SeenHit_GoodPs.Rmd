...
ggplot(OutsideObjectsMemory[!SubjectNo %in% bad.ps, .(MeanSeenHit = mean(SeenHit)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanSeenHit)) +
...
```{r, echo = FALSE}
t.test(MeanSeenHit ~ Group, data = OutsideObjectsMemory[!SubjectNo %in% bad.ps, .(MeanSeenHit = mean(SeenHit)), by = c("SubjectNo", "Group")], paired = TRUE)
```
The difference didn't reach significance. Now we have **16 good participants** so will see whether the effect will get larger when we have more participants. 
...
ggplot(Curiosity.Recall.Old[!SubjectNo %in% bad.ps], aes(x = ItemOrder, y = SeenHit)) +
...
```{r, echo = FALSE}
baseline.item <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo, data = Curiosity.Recall.Old[!SubjectNo %in% bad.ps], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(Curiosity.Recall.Old.Collapsed.Grp[!SubjectNo %in% bad.ps], aes(x = Group, y = SeenHit)) +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(SeenHit ~ Group * ItemOrderType, random = ~ Group + ItemOrderType | SubjectNo, data = Curiosity.Recall.Old.Collapsed.Grp[!SubjectNo %in% bad.ps], method = "ML")
...
ggplot(OutsideObjectsMemory[!SubjectNo %in% bad.ps], aes(x = CuriosityRating, y = SeenHit)) +
...
```{r, echo = FALSE}
baseline.rating <- lme(SeenHit ~ 1, random = ~ 1|SubjectNo, data = OutsideObjectsMemory[!SubjectNo %in% bad.ps], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(Curiosity.Recall.Old[!SubjectNo %in% bad.ps], aes(x = CuriosityRating, y = SeenHit)) +
...
### Collapse the order and curiosity rating
```{r, echo=FALSE, fig.align='center', fig.width=4.2, fig.height=3.6, fig.cap='Figure 7. Mean hit rate as a function of item order (Early vs. Late) and curiosity (Low vs. High).'}
ggplot(Curiosity.Recall.Old.Collapsed[!SubjectNo %in% bad.ps], aes(x = CuriosityRatingType, y = SeenHit)) +
  stat_summary(fun.y = mean, geom = "bar", size = 2, fill = "grey50") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.9)) +
  labs(x = "Curiosity rating", y = "Hit rate for recollection") +
  facet_wrap( ~ ItemOrderType, nrow = 1) +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(SeenHit ~ CuriosityRatingType * ItemOrderType, random = ~ CuriosityRatingType + ItemOrderType | SubjectNo, data = Curiosity.Recall.Old.Collapsed[!SubjectNo %in% bad.ps], method = "ML")
...
anova(baseline.rating, novelty.model.1, novelty.model.2, rating.model.1, rating.model.2, interaction.model)
```
