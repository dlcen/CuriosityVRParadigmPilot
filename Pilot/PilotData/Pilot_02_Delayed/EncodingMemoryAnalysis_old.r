...
InsideObjectsMemory[, .(MeanFalseAlarmRate = mean(ItemFAR, na.rm = T))]
InsideObjectsMemory[, .(MeanItemAccuracy = mean(ItemAccuracy, na.rm = T)), by = c("Group")]
InsideObjectsMemory[, .(MeanItemAccuracy = mean(ContextAccuracy, na.rm = T)), by = c("Group")]
...
ggplot(Encoding.Recall.Freq, aes(x = Response, y = Frequency, group = Group)) + theme_bw() +
