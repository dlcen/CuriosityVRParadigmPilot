...
ggplot(InsideObjectsMemory[, .(MeanSeenHit = mean(CorrSeenHit)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanSeenHit)) +
...
```{r, echo = FALSE}
t.test(MeanSeenHit ~ Group, data = InsideObjectsMemory[, .(MeanSeenHit = mean(CorrSeenHit)), by = c("SubjectNo", "Group")], paired = TRUE)
```
...
```
For both *familiar* and *novel* rooms, the hit rate seems to be higher at the two ends as compared to the middle. `r text_spec("Note that we only have 17 participants here", background = "#D05A6E", color = "white", bold = T, font_size = 15)`. 
...
In the complete model that contains all the predictors, none of the predictors has a significant estimated parameter value. It may be mainly we had only 11 participants now and less power due to small prediction-participant ratio.
