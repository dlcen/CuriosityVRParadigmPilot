...
```{r meanbars, echo=FALSE, fig.align='center', fig.width=3.6, fig.height=3, fig.cap='Figure 3. Mean accuracy for item memory respectively for Familiar and Novel rooms.'}
ggplot(InsideObjectsMemory[, .(MeanItemAccuracy = mean(ItemAccuracy)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanItemAccuracy)) +
  stat_summary(fun.y = mean, geom = "bar") +
  stat_summary(fun.data = mean_cl_boot, geom = "errorbar", width = 0.2) +
  labs(x = "Room novelty", y = "Mean accuracy for item memory")
```

The mean accuracy of item memory is almost the same between the *familiar* rooms than in the *novel* rooms. 

```{r, echo = FALSE}
t.test(MeanItemAccuracy ~ Group, data = InsideObjectsMemory[, .(MeanItemAccuracy = mean(ItemAccuracy)), by = c("SubjectNo", "Group")], paired = TRUE)
```

## Item memory as a function of item order
### Visualise the data
```{r, echo = FALSE, fig.align= 'center', fig.width=4.8, fig.height=3.6, fig.cap="Figure 4. Mean accuracy of item memory for items at each order (i.e., 1st, 2nd, 3rd, etc), respectively for familiar and novel rooms."}
ggplot(Encoding.Recall.Old, aes(x = ItemOrder, y = Accuracy)) +
...
```
For both *familiar* and *novel* rooms, the accuracy of item memory seems to be higher at the two ends as compared to the middle. `r text_spec("Note that we only have 25 participants here", background = "#D05A6E", color = "white", bold = T, font_size = 15)`. 
...
```{r, echo = FALSE}
baseline.item <- lme(Accuracy ~ 1, random = ~ 1|SubjectNo, data = Encoding.Recall.Old, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(Encoding.Recall.Old.Collapsed.Grp, aes(x = Group, y = Accuracy)) +
  ...
  labs(x = "Room Novelty", y = "Mean Accuracy for memory") +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(Accuracy ~ Group * ItemOrderType, random = ~ Group + ItemOrderType | SubjectNo, data = Encoding.Recall.Old.Collapsed.Grp, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
...
```{r, echo=FALSE, fig.align='center', fig.width=3.6, fig.height=3, fig.cap='Figure 5. Mean accuracy for item memory as a function of curiosity rating.'}
ggplot(InsideObjectsMemory, aes(x = CuriosityRating, y = ItemAccuracy)) +
...
```
There seems to be a weak positive correlation between curiosity rating and item memory **on average**.
...
```{r, echo = FALSE}
baseline.rating <- lme(ItemAccuracy ~ 1, random = ~ 1|SubjectNo, data = InsideObjectsMemory, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
```
After taking into account individual difference, the estimated slope (relationship between curiosity rating and item memory) is still weakly positive. 
...
ggplot(Encoding.Recall.Old, aes(x = CuriosityRating, y = Accuracy)) +
  ...
  theme(strip.text = element_text(face = "bold", size = 11),
        axis.title = element_text(face = "bold"))
```
For three of the six orders, there seems to be a positive correlates between curiosity rating and item memory.
...
```
Including none of the predictors improved the model fit significantly. 
...
ggplot(Encoding.Recall.Old.Collapsed, aes(x = CuriosityRatingType, y = Accuracy)) +
  stat_summary(fun.y = mean, geom = "bar", size = 2, fill = "grey50") +
  stat_summary(fun.data = mean_cl_normal, geom = "errorbar", width = 0.2, position = position_dodge(width = 0.9)) +
  labs(x = "Curiosity rating", y = "Mean accuracy for item memory") +
  facet_wrap( ~ ItemOrderType, nrow = 1) +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(Accuracy ~ CuriosityRatingType * ItemOrderType, random = ~ CuriosityRatingType + ItemOrderType | SubjectNo, data = Encoding.Recall.Old.Collapsed, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
