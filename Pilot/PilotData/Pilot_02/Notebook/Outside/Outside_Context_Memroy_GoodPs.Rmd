...
OutsideObjectsMemory$Type <- "Good"
OutsideObjectsMemory[SubjectNo %in% bad.ps]$Type <- "Bad"

Curiosity.Recall.Old.Corr.Freq$Type <- "Good"
Curiosity.Recall.Old.Corr.Freq[SubjectNo %in% bad.ps]$Type <- "Bad"
```

# Individual performance
## An overview of individual context memory performance as a function of room.
```{r, echo=FALSE, fig.align='center', fig.width=7.2, fig.height=7.2, fig.cap='Figure 1. Mean accuracy for context memory respectively for each room and each individual participant.'}
ggplot(OutsideObjectsMemory, aes(x = Room, y = ContextAccuracy)) +
  geom_rect(data = subset(OutsideObjectsMemory,Type == 'Bad'),aes(fill = Type),xmin = -Inf,xmax = Inf,ymin = -Inf,ymax = Inf,alpha = 0.1) +
...
```{r, echo=FALSE, fig.align='center', fig.width=7.2, fig.height=7.2, fig.cap='Figure 3. Frequency of correctly labelled distractor objects for each individual participant.'}
ggplot(Curiosity.Recall.Old.Corr.Freq, aes(x = Response, y = Frequency)) +
  geom_rect(data = subset(Curiosity.Recall.Old.Corr.Freq,Type == 'Bad'),aes(fill = Type),xmin = -Inf,xmax = Inf,ymin = -Inf,ymax = Inf,alpha = 0.1) +
  geom_line( aes(group = Group, colour = Group)) +
...
ggplot(OutsideObjectsMemory[!SubjectNo %in% bad.ps, .(MeanContextAccuracy = mean(ContextAccuracy)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanContextAccuracy)) +
...
```{r, echo = FALSE}
t.test(MeanContextAccuracy ~ Group, data = OutsideObjectsMemory[!SubjectNo %in% bad.ps, .(MeanContextAccuracy = mean(ContextAccuracy)), by = c("SubjectNo", "Group")], paired = TRUE)
```
...
ggplot(Curiosity.Recall.Old[!SubjectNo %in% bad.ps], aes(x = ItemOrder, y = ContextAccuracy)) +
...
```{r, echo = FALSE}
baseline.context <- lme(ContextAccuracy ~ 1, random = ~ 1|SubjectNo, data = Curiosity.Recall.Old[!SubjectNo %in% bad.ps], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(Curiosity.Recall.Old.Collapsed.Grp[!SubjectNo %in% bad.ps], aes(x = Group, y = ContextAccuracy)) +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(ContextAccuracy ~ Group * ItemOrderType, random = ~ Group + ItemOrderType | SubjectNo, data = Curiosity.Recall.Old.Collapsed.Grp[!SubjectNo %in% bad.ps], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE))
...
ggplot(OutsideObjectsMemory[!SubjectNo %in% bad.ps], aes(x = CuriosityRating, y = ContextAccuracy)) +
...
```{r, echo = FALSE}
baseline.rating <- lme(ContextAccuracy ~ 1, random = ~ 1|SubjectNo, data = OutsideObjectsMemory[!SubjectNo %in% bad.ps], method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
ggplot(Curiosity.Recall.Old[!SubjectNo %in% bad.ps], aes(x = CuriosityRating, y = ContextAccuracy)) +
