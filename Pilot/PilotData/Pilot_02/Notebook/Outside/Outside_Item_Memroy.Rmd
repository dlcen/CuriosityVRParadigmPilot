...
```{r, echo=FALSE, fig.align='center', fig.width=6.4, fig.height=5.4, fig.cap='Figure 1. Response frequency for new and old items respectively for each individual participant.'}
...
```{r, echo=FALSE, fig.align='center', fig.width=7.2, fig.height=5.4, fig.cap='Figure 2. Response frequency for new and old items respectively for each Familiar/Novel group and each individual participant.'}
...
ggplot(OutsideObjectsMemory[, .(MeanItemAccuracy = mean(ItemAccuracy)), by = c("SubjectNo", "Group")], aes(x = Group, y = MeanItemAccuracy)) +
...
```{r, echo = FALSE}
t.test(MeanItemAccuracy ~ Group, data = OutsideObjectsMemory[, .(MeanItemAccuracy = mean(ItemAccuracy)), by = c("SubjectNo", "Group")], paired = TRUE)
```
The difference didn't reach significance, but we only had 17 participants. 
...
ggplot(Curiosity.Recall.Old, aes(x = ItemOrder, y = Accuracy)) +
...
```
For *familiar* rooms the accuracy of item memory increases with the item order, but the pattern was not obvious for the *novel* rooms. `r text_spec("Note that we only have 11 participants here", background = "#D05A6E", color = "white", bold = T, font_size = 15)`. 
...
```{r, echo = FALSE}
baseline.item <- lme(Accuracy ~ 1, random = ~ 1|SubjectNo, data = Curiosity.Recall.Old, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
In the complete model that contains all the predictors, none of the predictors has a significant estimated parameter value. It may be mainly we had only 11 participants now and less power due to small prediction-participant ratio. Below I will examine the optimal model `order.model.quad.1`. It's considered as "optimal" because adding more predictors didn't improve its model fit.
...
ggplot(OutsideObjectsMemory, aes(x = CuriosityRating, y = ItemAccuracy)) +
...
```
Against our hypothesis, there is a *negative* correlation between curiosity rating and accuracy of item memory.
...
```{r, echo = FALSE}
baseline.rating <- lme(ItemAccuracy ~ 1, random = ~ 1|SubjectNo, data = OutsideObjectsMemory, method = "ML", control = list(msMaxIter = 500, opt = "optim", msVerbose=FALSE), na.action = na.exclude)
...
```
There is a negative but not significant correlation between curiosity rating and item memory.
...
ggplot(Curiosity.Recall.Old, aes(x = CuriosityRating, y = Accuracy)) +
  ...
  theme(strip.text = element_text(face = "bold", size = 11),
        axis.title = element_text(face = "bold"))
```
For 4 of the orders, there was a negative trend but for the other half, the trend looks positive.
...
```
Again, adding none of the predictors significantly improved the model fit.
