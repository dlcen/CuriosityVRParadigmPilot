...
```
The difference didn't reach significance. Now we have **21 participants** so will see whether the effect will get larger when we have more participants. 
...
```
For *familiar* rooms the accuracy of item memory increases with the item order, but the pattern was not obvious for the *novel* rooms. `r text_spec("Note that we only have 21 participants here", background = "#D05A6E", color = "white", bold = T, font_size = 15)`. 
...
In the complete model that contains all the predictors, none of the predictors has a significant estimated parameter value. It may be mainly we had only 21 participants now and less power due to small prediction-participant ratio. Below I will examine the optimal model `order.model.linear.2`. It's considered as "optimal" because adding more predictors didn't improve its model fit.

#### Parameter estimation for the optimal model
```{r, echo=FALSE}
summary(order.model.linear.2, correlation = FALSE)
```
...
ggplot(Curiosity.Recall.Old.Collapsed.Grp, aes(x = Group, y = SeenHit)) +
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(SeenHit ~ Group * ItemOrderType, random = ~ Group + ItemOrderType | SubjectNo, data = Curiosity.Recall.Old.Collapsed.Grp, method = "ML")
...
#### Stats on it
```{r, echo = FALSE}
repeatedModel <- lme(SeenHit ~ CuriosityRatingType * ItemOrderType, random = ~ CuriosityRatingType + ItemOrderType | SubjectNo, data = Curiosity.Recall.Old.Collapsed, method = "ML")
