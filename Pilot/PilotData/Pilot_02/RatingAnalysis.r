...
GroupA <- c(13:20, 30, 33:34, 37:38, 41:42, 45:46, 49:50, 53:54, 57:58)
GroupB <- c(21:29, 31:32, 35:36, 39:40, 43:44, 47:48, 51:52, 55:56)
...
### > maybe using median instead of mean? as there are only seven numbers? For those lower than the median defined as "Low" curiosity and higher -> "high" curiosity.
### > Also, maybe can get quartiles.
...
# Curiosity.Ratings.ordered     <- Curiosity.Ratings[order(SubjectNo, Room), ]
# Curiosity.Ratings.ordered.mlt <- Curiosity.Ratings.ordered[!(SubjectNo == "P30" & Room == "LivingRoom")][rep(1:nrow(Curiosity.Ratings.ordered[!(SubjectNo == "P30" & Room == "LivingRoom")]), each = 6), ]
# temp <- Curiosity.Ratings.ordered[SubjectNo == "P30" & Room == "LivingRoom"][rep(1:nrow(Curiosity.Ratings.ordered[SubjectNo == "P30" & Room == "LivingRoom"]), each = 2), ]
# Curiosity.Ratings.ordered.mlt <- rbind(Curiosity.Ratings.ordered.mlt, temp)
# Curiosity.Ratings.ordered.mlt <- Curiosity.Ratings.ordered.mlt[order(SubjectNo, Room), ]
# 
# Encoding.Recall.Old$CuriosityRating <- Curiosity.Ratings.ordered.mlt$CuriosityRating
# 
# Curiosity.Ratings.ordered     <- Curiosity.Ratings[order(SubjectNo, Room), ]
# Curiosity.Ratings.ordered.mlt <- Curiosity.Ratings.ordered[rep(1:nrow(Curiosity.Ratings.ordered), each = 6), ]
# Curiosity.Recall.Old$CuriosityRating <- Curiosity.Ratings.ordered.mlt$CuriosityRating
