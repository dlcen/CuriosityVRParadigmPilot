...
```{r}
library(data.table); library(nlme); library(multcomp); library(ggplot2)
```
...
```{r}
cur.model <- update(baseline, .~. + Curiosity, random = ~ 1 + Curiosity|SubjectNo)
```
...
```{r}
sur.model <- update(baseline, .~. + Surprise, random = ~ 1 + Surprise|SubjectNo)
```

```{r}
anova(baseline, sur.model)
```
...
```{r, echo=FALSE}
outside.hit.rate.per.room[, PredSur := predict(sur.model, level = 1)]
```
...
```{r, echo=FALSE}
ggsave("../Figures/SurHitLinear.png", width = 6.4, height = 4.8 )
```
...
ggplot(outside.hit.rate.per.room, aes(x = PreSur, y = PredPreSur)) +
  ...
  labs(x = "Surprise score for the previous trial (Interest - Curiosity)", y = "Corrected hit rate (%)" )+
  theme(legend.position = "none",
        axis.title = element_text(face = "bold", size = 14),
        axis.text = element_text(size = 12))
```
...
```{r, echo=FALSE}
ggsave("../Figures/PreSurHitLinear.png", width = 6.4, height = 4.8 )
```
